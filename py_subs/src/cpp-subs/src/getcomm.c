...
extern "C" {
   ...
   *#ifndef lint
   *static char     rcsid[] =
   *"$Id: getcomm.c,v 3.14 1994/09/26 22:56:19 thewalt Exp $";
...
char           *getcomm();		/* read a line of input */
...
/* set up variables and terminal */
{
	...
	gl_error("\n*** Error: getcomm(): not interactive, use stdio.\n");
    gl_char_init();
    gl_init_done = 1;
}
...
/* adds the character c to the input buffer at current location */
{
	...
	gl_error("\n*** Error: getcomm(): input buffer overflow\n");
...
char *buf;
{
    ...
    /* in case we call gl_histadd() before we call getcomm() */
    if (gl_init_done < 0) {		/* -1 only on startup */
        hist_init();
        gl_init_done = 0;
    }
